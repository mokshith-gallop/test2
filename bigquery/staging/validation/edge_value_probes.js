// Edge-value round-trip probes for AC-5
//
...
//
// DECIMAL(14,2) → NUMERIC probes (cleansed_orders.gross_amount, net_amount):
//   - 999999999999.99  (max positive 14,2)
//   - -999999999999.99 (max negative 14,2)
//   - 0.01             (minimum non-zero)
//   - 0.00             (exact zero)
//
// DOUBLE → FLOAT64 probes (cleansed_customers.geocoded_lat, geocoded_lon):
//   - NaN
//   - +Infinity
//   - -Infinity
//   - -0.0
...
async function main() {
  ...
  console.log('Edge-Value Round-Trip Probes (AC-5)');
  console.log('====================================\n');
  ...
  // ─── Phase 1: DECIMAL(14,2) → NUMERIC round-trip ─────────────────
  console.log('--- DECIMAL(14,2) → NUMERIC probes ---');
  ...
  try {
    ...
    await query(`
      INSERT INTO ${scratchNumeric} (probe_label, gross_amount, net_amount) VALUES
        ('max_positive', NUMERIC '999999999999.99', NUMERIC '999999999999.99'),
        ('max_negative', NUMERIC '-999999999999.99', NUMERIC '-999999999999.99'),
    ...
    const expected = {
      ...
      'max_negative': { gross: '-999999999999.99', net: '-999999999999.99' },
      'max_positive': { gross: '999999999999.99', net: '999999999999.99' },
  ...
  // ─── Phase 2: DOUBLE → FLOAT64 round-trip ────────────────────────
  console.log('\n--- DOUBLE → FLOAT64 probes ---');
  ...
  try {
    ...
    console.error(`  ✗ FLOAT64 probe setup failed: ${err.message}`);
    failed += 4; // All 4 probes failed
  }

  // ─── Phase 3: Cleanup scratch tables ──────────────────────────────
  console.log('\n--- Cleaning up scratch tables ---');
  for (const tbl of ['edge_probe_numeric', 'edge_probe_float64']) {
    try {
